...
// `tools/dxf_pipeline/build_profiles_index.py` depuis les SKU
// `statut_gate == "OK"`, cardinalité `kExpectedGateOkCount`), PAS
// d'AssetManifest.json.
//
// ⚠️ CE N'EST PAS UNE EXTENSION DE COUVERTURE, c'est une RESTRICTION
// (56 -> kExpectedGateOkCount) : `D830` (gate-OK) ET `D705` (gate-OK)
// étaient TOUS LES DEUX déjà présents dans `AssetManifest.json` avant ce
// commit (fichiers `statut: OK` sur disque depuis le batch Piste A),
// donc DEJA "couverts" et rendus en métrique en build RELEASE par
// l'ancien mécanisme (le contrôle bbox_mm n'était qu'un `assert()`,
// absent du binaire release). Ce test ne vérifie donc PAS que D830
// devient nouvellement accessible -- il vérifie que le nouveau
// mécanisme (index.json, gate-OK) couvre D830 pour la BONNE raison
// structurelle (gate), et non plus par la coïncidence de l'ancien
// mécanisme (AssetManifest.json, 56 statut:OK sans distinction).
//
...
//
// `kExpectedGateOkCount` centralisé dans `gate_ok_count_helper.dart` --
// voir ce fichier pour l'historique du chiffre (31 -> 39 le
// 2026-08-26, patch two-pass sweep + batch22).
library;
...
void main() {
  ...
  test(
    'D614 (statut OK, fichier existe, mais HORS des SKU gate-OK) ne doit '
  ...
  test(
    ...
    'de index.json (kExpectedGateOkCount refs) et non d\'AssetManifest'
    '.json (56 fichiers statut:OK) -- coveredRefsForTesting doit avoir '
    'EXACTEMENT kExpectedGateOkCount elements apres resolution, jamais '
    '56. Necessaire car aucune divergence gate/loader n\'existe '
    'empiriquement sur le jeu de donnees actuel (verifie separement) -- '
    'cette garantie doit tenir independamment de cette coincidence, y '
    'compris si le gate durcit ses criteres demain sans que le loader '
    'change.',
    () async {
      final completer = Completer<void>();
      void cb() {
